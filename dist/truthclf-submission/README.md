...
All numbers are **calibrated**: a post-hoc calibrator (the better of Platt /
temperature scaling, selected on validation) plus a decision threshold, both fit
on a separate validation split and reported on the untouched test split.
...
Tested dependency versions: numpy 2.0.2, together 2.9.0, python-dotenv,
pandas 2.3.3, matplotlib 3.9.4.
...
with bootstrap CIs. The binarization is configurable: a `sensitivity` scheme
(`half-true → False`) is implemented (and unit-tested), with its effect on the
class balance shown in the EDA (`scripts/analyze_dataset.py`).
...
conflicting binary label). The speaker-disjoint split is what we report; a
stratified-random split is also implemented for comparison (the EDA quantifies the
heavy speaker concentration that motivates the disjoint choice).

**Calibration & thresholds.** Both elicitations emit a probability — `p(True)`
from the True/False token logprobs (the primary baseline) or the `0–100 score / 100`
(secondary) — and both are uncalibrated, so we fit a post-hoc calibrator (the
better of Platt / temperature scaling, selected on validation) and a decision
threshold on validation only, then report on test; calibration reduces ECE without
changing accuracy (it is monotonic). A cost-sensitive threshold is also provided
because, in a misinformation setting, labelling a false statement as True (a false
positive in this 1=True encoding) is the costlier error.

**Fine-tuning.** LoRA supervised fine-tuning (SFT) on Together. The single labelled
`training_dataset` is split speaker-disjointly into train/validation internally;
the prompt is serialized as system+user and the assistant target is a single
`True`/`False` token (loss on the label only, `train_on_inputs="auto"`). Serverless
serving of *custom* LoRA adapters was not available for our base, so the fine-tuned
model is served on a **short-lived dedicated endpoint** purely for evaluation —
created, queried, and deleted within minutes, with a guaranteed-teardown safeguard
(deletion in a `finally` block plus a max-uptime kill switch).
...
agreement rate, which surfaces cases where the model's stated reason does not match
the input that actually moved its prediction.
...
truthclf/
  ...
  llm.py         Together clients (sync + Batch API), on-disk response cache
  ...
  predictors/
    ...
    zeroshot.py  ZeroShotPredictor (logprob/decision primary; 0-100 score secondary)
